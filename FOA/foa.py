...
from cli_utils import accuracy, AverageMeter
...
class FOA(nn.Module):
    ...
    def __init__(self, model:PromptViT, fitness_lambda=0.4):
        print("-" * 30)
        print("Within the init() of FOA")
        print(f"Base Model : PromptViT | fitness_lambda = {fitness_lambda} | Prompts : {model.prompts}")
        super().__init__()
        ...
        print(f"Exiting the init() of FOA")
        print("-" * 30)

    def _init_cma(self):
        """CMA-ES initialization"""
        print("-" * 30)
        print("Within the init_cma function")
        ...
        print(f"Setting the CMA Evolution Strategy with options : {cma_opts}")
        es = cma.CMAEvolutionStrategy(dim * [0], 1, inopts=cma_opts)
        self.popsize = es.popsize
        print(f"Exiting the init_cma() with ES = {es}")
        print("-" * 30)
        return es
    ...
    def forward(self, x):
        ...
        print("-" * 30)
        print(f"Within the forward function for current batch")
        print(f"Getting the Shift Direction")
        ...
        for j, prompt in enumerate(prompts):
            ...
            print(f"Prompts after adding to the model = {self.model.prompts}")
            print(f"Getting the Loss Value after forward pass")
...
def forward_and_get_loss(images, model:PromptViT, fitness_lambda, train_info, shift_vector, imagenet_mask):
    print("-" * 30)
    print("Within the forward_and_get_loss function")
    ...
    print(f"Discrepancy Loss = {discrepancy_loss}")
    ...
    if shift_vector is not None:
        ...
        if imagenet_mask is not None:
            output = output[:, imagenet_mask]
    print(f"Output = {output} | Loss = {loss} | batch_mean = {batch_mean}")
    print(f"Exiting the forward_and_get_loss function")
    print("-" * 30)
